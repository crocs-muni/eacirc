...
{
  if(pGlobals->settings->heatMap.enabledMask & 0x0010){
...
void decim_lfsr_filter(DECIM_ctx *ctx)
{
  u8 b,c;
      ...
      c = ctx->lfsr_state[1];
      b = ctx->lfsr_state[21];
        if(pGlobals->settings->heatMap.enabledMask & 0x0001){
        c ^= ctx->lfsr_state[21];
        b += ctx->lfsr_state[39];
        c ^= ctx->lfsr_state[39];
        b += ctx->lfsr_state[51];
        ...
        if(pGlobals->settings->heatMap.enabledMask & 0x0002){
            if(pGlobals->settings->heatMap.enabledMask & 0x0001){
  ...
  /* compute the next bit for the LFSR*/
      ...
      if(pGlobals->settings->heatMap.enabledMask & 0x0008){
          if(pGlobals->settings->heatMap.enabledMask & 0x0004){
...
{
    if(pGlobals->settings->heatMap.enabledMask & 0x0020) ctx->bit_searched = (ctx->searching ? ctx->bit_searched : bit);
    if(pGlobals->settings->heatMap.enabledMask & 0x0040) ctx->out  = ctx->searching & (ctx->immediate_finding ^ bit);
    if(pGlobals->settings->heatMap.enabledMask & 0x0080) ctx->immediate_finding = ctx->searching & (ctx->bit_searched ^ bit);
    if(pGlobals->settings->heatMap.enabledMask & 0x0100) ctx->searching  = !ctx->searching |ctx->immediate_finding;
}
