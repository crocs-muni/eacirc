...
/* ------------------------------------------------------------------------- */
class ECRYPT_Sfinks: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define SFINKS_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  SFINKS_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define SFINKS_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  SFINKS_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void SFINKS_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  SFINKS_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define SFINKS_keystream_blocks(ctx, keystream, blocks)            \
	  SFINKS_keystream_bytes(ctx, keystream,                           \
		(blocks) * SFINKS_BLOCKLENGTH)
	...
	void SFINKS_encrypt_blocks(
	  SFINKS_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void SFINKS_decrypt_blocks(
	  SFINKS_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
