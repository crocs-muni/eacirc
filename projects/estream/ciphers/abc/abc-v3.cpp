 ...
 * above.
 */
void ECRYPT_ABC::ECRYPT_keysetup(
  void* ctxa,
  const u8* key,
 ...
 * IV's.
 */
void ECRYPT_ABC::ECRYPT_ivsetup(
  void* ctxa,
  const u8* iv)
{
	ABC_ctx* ctx = (ABC_ctx*)ctxa;
 ...
 */
void ECRYPT_ABC::ABC_process_bytes(
 ...
 */
void ECRYPT_ABC::ABC_keystream_bytes(
  ABC_ctx* ctx,
  u8* keystream,
 ...
 */
void ECRYPT_ABC::ABC_process_blocks(
  int action,                 /* 0 = encrypt; 1 = decrypt; */
  ABC_ctx* ctx,
  const u8* input,
  u8* output,
 ...
 */
void ECRYPT_ABC::ABC_keystream_blocks(
  ABC_ctx* ctx,
  u8* keystream,
...
void ECRYPT_ABC::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	ABC_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_ABC::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	ABC_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}