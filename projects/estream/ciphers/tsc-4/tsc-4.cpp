...
void ECRYPT_Tsc4::ECRYPT_keysetup(
  void* ctxa,
  const u8* key,
  u32 keysize,
  u32 ivsize)
...
void ECRYPT_Tsc4::ECRYPT_ivsetup(
  void* ctxa, 
  const u8* iv)
{
	TSC4_ctx* ctx = (TSC4_ctx*)ctxa;
  ...
  u32 msglen)
{
	...
	for(i=0; i < msglen; i ++)
	{
		tmp = tsc(ctx, numRounds);
		output[i] = input[i] ^ tmp;
	}
}

void ECRYPT_Tsc4::TSC4_keystream_bytes(
  TSC4_ctx* ctx,
  u8* keystream,
  u32 length)
{
...
void ECRYPT_Tsc4::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	TSC4_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Tsc4::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	TSC4_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}