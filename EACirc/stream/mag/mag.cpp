...
void ECRYPT_Mag::ECRYPT_keysetup(
  void* ctxa, 
  const u8* key, 
  u32 keysize,               
  u32 ivsize)
...
void ECRYPT_Mag::ECRYPT_ivsetup(void* ctxa,const u8* iv)
{
	MAG_ctx* ctx = (MAG_ctx*)ctxa;
  int i;
...
void ECRYPT_Mag::MAG_process_bytes(
...
void ECRYPT_Mag::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	MAG_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Mag::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	MAG_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}