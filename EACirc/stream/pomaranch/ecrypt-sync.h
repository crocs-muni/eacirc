...
/* ------------------------------------------------------------------------- */
class ECRYPT_Pomaranch: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	void POMARANCH_keystream_bytes(
	  POMARANCH_ctx* ctx,
	  u8* keystream,
	...
	#define POMARANCH_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  POMARANCH_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define POMARANCH_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  POMARANCH_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void POMARANCH_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  POMARANCH_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define POMARANCH_keystream_blocks(ctx, keystream, blocks)            \
	  POMARANCH_keystream_bytes(ctx, keystream,                           \
		(blocks) * POMARANCH_BLOCKLENGTH)
	...
	void POMARANCH_encrypt_blocks(
	  POMARANCH_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void POMARANCH_decrypt_blocks(
	  POMARANCH_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
	...
	void POMARANCH_keystream_blocks(
	  POMARANCH_ctx* ctx,
	  u8* keystream,
