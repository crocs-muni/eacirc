...
static void genrand_bytes_first(CRYPTMT_ctx * ctx, u8 cipher[], const u8 plain[],
				u32 len);
static void fast_genrand_bytes_first(CRYPTMT_ctx * ctx, u8 cipher[],
				     const u8 plain[], u32 len);
static void genrand_bytes(CRYPTMT_ctx * ctx, u8 cipher[], const u8 plain[], 
			  u32 len);
static INLINE void fast_genrand_bytes(CRYPTMT_ctx * ctx, u8 cipher[],
				      const u8 plain[], u32 len);
static void genrand_block_first(CRYPTMT_ctx * ctx, u8 cipher[], 
				const u8 plain[]);
static void fast_genrand_block_first(CRYPTMT_ctx * ctx, u8 cipher[], 
					    const u8 plain[]);
static INLINE void genrand_block(CRYPTMT_ctx * ctx, u8 cipher[],
				 const u8 plain[]);
...
/* Key size in bits. */
/* IV size in bits. */
void ECRYPT_Cryptmt::ECRYPT_keysetup(void * ctxa, const u8 * key, u32 keysize,
		     u32 ivsize)
{
	CRYPTMT_ctx* ctx = (CRYPTMT_ctx*)ctxa;
    s32 i;
...
void ECRYPT_Cryptmt::ECRYPT_ivsetup(void * ctxa, const u8 * iv)
{
	CRYPTMT_ctx* ctx = (CRYPTMT_ctx*)ctxa;
...
void ECRYPT_Cryptmt::ECRYPT_encrypt_bytes(void * ctxa,
			  const u8 * plaintext,
...
/* Message length in bytes. */
void ECRYPT_Cryptmt::ECRYPT_decrypt_bytes(void * ctxa,
			  const u8 * ciphertext,
			  u8 * plaintext, u32 msglen)
{
    ECRYPT_encrypt_bytes(ctxa, ciphertext, plaintext, msglen);
}
