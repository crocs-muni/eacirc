...
#ifdef SOSEMANUK_ECRYPT
void
ECRYPT_Sosemanuk::ECRYPT_init(void)
{
	return;
}
#endif
...
#ifdef SOSEMANUK_ECRYPT
void
ECRYPT_Sosemanuk::ECRYPT_keysetup(void *kca, const u8 *key, u32 keysize, u32 ivsize)
#else
/* see sosemanuk.h */
void
...
#endif
{
	SOSEMANUK_ctx* kc = (SOSEMANUK_ctx*)kca;
	/*
...
#ifdef SOSEMANUK_ECRYPT
void
ECRYPT_Sosemanuk::ECRYPT_ivsetup(void *ctxa, const u8 *iv)
#else
/* see sosemanuk.h */
void
...
#define FSF(zc, S, i0, i1, i2, i3, i4, o0, o1, o2, o3)  do { \
	 ...
	 */
	if (numRounds < 2) FSS(0, S0, 0, 1, 2, 3, 4, 1, 4, 2, 0);
	if (numRounds < 3) FSS(4, S1, 1, 4, 2, 0, 3, 2, 1, 0, 4);
	if (numRounds < 4) FSS(8, S2, 2, 1, 0, 4, 3, 0, 4, 1, 3);
	if (numRounds < 5) FSS(12, S3, 0, 4, 1, 3, 2, 4, 1, 3, 2);
	if (numRounds < 6) FSS(16, S4, 4, 1, 3, 2, 0, 1, 0, 4, 2);
	if (numRounds < 7) FSS(20, S5, 1, 0, 4, 2, 3, 0, 2, 1, 4);
	if (numRounds < 8) FSS(24, S6, 0, 2, 1, 4, 3, 0, 2, 3, 1);
	if (numRounds < 9) FSS(28, S7, 0, 2, 3, 1, 4, 4, 1, 2, 0);
	if (numRounds < 10) FSS(32, S0, 4, 1, 2, 0, 3, 1, 3, 2, 4);
	if (numRounds < 11) FSS(36, S1, 1, 3, 2, 4, 0, 2, 1, 4, 3);
	if (numRounds < 12) FSS(40, S2, 2, 1, 4, 3, 0, 4, 3, 1, 0);
	if (numRounds < 13) FSS(44, S3, 4, 3, 1, 0, 2, 3, 1, 0, 2);
	rc->s09 = r3;
	rc->s08 = r1;
	rc->s07 = r0;
	rc->s06 = r2;

	if (numRounds < 14) FSS(48, S4, 3, 1, 0, 2, 4, 1, 4, 3, 2);
	if (numRounds < 15) FSS(52, S5, 1, 4, 3, 2, 0, 4, 2, 1, 3);
	if (numRounds < 16) FSS(56, S6, 4, 2, 1, 3, 0, 4, 2, 0, 1);
	if (numRounds < 17) FSS(60, S7, 4, 2, 0, 1, 3, 3, 1, 2, 4);
	if (numRounds < 18) FSS(64, S0, 3, 1, 2, 4, 0, 1, 0, 2, 3);
	if (numRounds < 19) FSS(68, S1, 1, 0, 2, 3, 4, 2, 1, 3, 0);
	rc->r1  = r2;
	rc->s04 = r1;
	rc->r2  = r3;
	rc->s05 = r0;

	if (numRounds < 20) FSS(72, S2, 2, 1, 3, 0, 4, 3, 0, 1, 4);
	if (numRounds < 21) FSS(76, S3, 3, 0, 1, 4, 2, 0, 1, 4, 2);
	if (numRounds < 22) FSS(80, S4, 0, 1, 4, 2, 3, 1, 3, 0, 2);
	if (numRounds < 23) FSS(84, S5, 1, 3, 0, 2, 4, 3, 2, 1, 0);
	if (numRounds < 24) FSS(88, S6, 3, 2, 1, 0, 4, 3, 2, 4, 1);
	if (numRounds < 25) FSF(92, S7, 3, 2, 4, 1, 0, 0, 1, 2, 3);
	rc->s03 = r0;
	rc->s02 = r1;
	rc->s01 = r2;
	rc->s00 = r3;
...
static void
sosemanuk_internal(SOSEMANUK_ctx *rc, const u32 *src, u32 *dst)
...
/* see ecrypt-sync.h */
void
ECRYPT_Sosemanuk::SOSEMANUK_process_bytes(int action, void *ctxa,
...
/* see ecrypt-sync.h */
void
ECRYPT_Sosemanuk::SOSEMANUK_process_blocks(int action, SOSEMANUK_ctx *ctx,
...
void ECRYPT_Sosemanuk::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	SOSEMANUK_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Sosemanuk::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
