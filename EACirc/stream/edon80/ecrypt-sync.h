...
/* ------------------------------------------------------------------------- */
class ECRYPT_Edon80: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define EDON80_keystream_blocks(ctx, keystream, blocks)            \
	  EDON80_keystream_bytes(ctx, keystream,                        \
		(blocks) * EDON80_BLOCKLENGTH)
	...
	void EDON80_encrypt_blocks(
	  EDON80_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void EDON80_decrypt_blocks(
	  EDON80_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
