...
void ECRYPT_Trivium::ECRYPT_keysetup(
  void* ctxa, 
  const u8* key, 
  u32 keysize,
  u32 ivsize)
...
void ECRYPT_Trivium::ECRYPT_ivsetup(
  void* ctxa, 
  const u8* iv)
{
  TRIVIUM_ctx* ctx = (TRIVIUM_ctx*)ctxa;
...
void ECRYPT_Trivium::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	TRIVIUM_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Trivium::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
