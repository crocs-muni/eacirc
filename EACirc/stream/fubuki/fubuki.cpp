...
void hmnencode(FUBUKI_ctx *ctx, 
	       const u8* plaintext, u8* ciphertext,  u32 msglen, int numRounds = Iteration)  /* Message length in bytes. */ 
{
    s32 i, j, repeat; 
    u32 msgbuf[Tuple];
    u32 cinpos, coutpos;
...
void hmndecode(FUBUKI_ctx *ctx, 
	       const u8* ciphertext, u8* plaintext, u32 msglen, int numRounds = Iteration) /* Message length in bytes. */ 
{
    s32 i, j, k, repeat;
...
void ECRYPT_Fubuki::ECRYPT_keysetup(
  void* ctxa, 
  const u8* key, 
  u32 keysize,               /* Key size in bits. */ 
  u32 ivsize)                /* IV size in bits. */ 
{
	FUBUKI_ctx* ctx = (FUBUKI_ctx*)ctxa;
    s32 i;
...
void ECRYPT_Fubuki::ECRYPT_ivsetup(
  void* ctxa, 
  const u8* iv)
{
	FUBUKI_ctx* ctx = (FUBUKI_ctx*)ctxa;
...
void ECRYPT_Fubuki::ECRYPT_encrypt_bytes(
  void* ctxa, 
  const u8* plaintext, 
  u8* ciphertext, 
...
{
	FUBUKI_ctx* ctx = (FUBUKI_ctx*)ctxa;
    hmnencode(ctx, plaintext, ciphertext, msglen, numRounds);
}

void ECRYPT_Fubuki::ECRYPT_decrypt_bytes(
  void* ctxa, 
  const u8* ciphertext, 
  u8* plaintext, 
  u32 msglen)                /* Message length in bytes. */ 
{
	...
	hmndecode(ctx, ciphertext, plaintext, msglen, numRounds);
}
