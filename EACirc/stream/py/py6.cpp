 ...
 */
void ECRYPT_Py::ECRYPT_keysetup(
  void* ctxa, 
  const u8* key, 
...
void ECRYPT_Py::PY_process_bytes(
...
void ECRYPT_Py::PY_keystream_bytes(
  PY_ctx* ctx,
  u8* keystream,
...
void ECRYPT_Py::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	PY_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Py::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	PY_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}