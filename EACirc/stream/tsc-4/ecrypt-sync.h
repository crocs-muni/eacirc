...
/* ------------------------------------------------------------------------- */
class ECRYPT_Tsc4: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define TSC4_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  TSC4_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define TSC4_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  TSC4_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void TSC4_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  TSC4_ctx* ctx,
	  const u8* iv,
	  const u8* input,
	...
	#define TSC4_keystream_blocks(ctx, keystream, blocks)            \
	  TSC4_keystream_bytes(ctx, keystream,                           \
		(blocks) * TSC4_BLOCKLENGTH)
	...
	void TSC4_encrypt_blocks(
	  TSC4_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void TSC4_decrypt_blocks(
	  TSC4_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
