...
void ECRYPT_Mickey::ECRYPT_keysetup(
  void* ctxa, 
  const u8* key, 
...
void ECRYPT_Mickey::ECRYPT_ivsetup(
  void* ctxa, 
  const u8* iv)
{
	MICKEY_ctx* ctx = (MICKEY_ctx*)ctxa;
...
void ECRYPT_Mickey::MICKEY_process_bytes(
...
void ECRYPT_Mickey::MICKEY_keystream_bytes(
  MICKEY_ctx* ctx,
  u8* keystream,
...
void ECRYPT_Mickey::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	MICKEY_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Mickey::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	MICKEY_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}