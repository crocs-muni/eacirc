...
int EstreamProject::loadProjectConfiguration(TiXmlNode* pRoot) {
    ...
    estreamSettings.testVectorBalance = atoi(getXMLElementValue(pRoot,"ESTREAMS/BALLANCED_TEST_VECTORS").c_str());
...
void EstreamProject::getTestVector(){
    ...
    switch (pEstreamSettings->testVectorEstreamMethod) {
        case TESTVECT_ESTREAM_DISTINCT:
            ...
            if (pEstreamSettings->testVectorBalance && (numstats[0] >= pGACirc->numTestVectors/2))
                streamnum = 1;
            else if (pEstreamSettings->testVectorBalance && (numstats[1] >= pGACirc->numTestVectors/2))
                streamnum = 0;
            else
