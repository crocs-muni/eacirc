...
/* ------------------------------------------------------------------------- */
class ECRYPT_Hermes: public EstreamInterface
{
public:
	...
	#define HERMES_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  HERMES_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define HERMES_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  HERMES_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void HERMES_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  HERMES_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define HERMES_keystream_blocks(ctx, keystream, blocks)            \
	  HERMES_keystream_bytes(ctx, keystream,                           \
		(blocks) * HERMES_BLOCKLENGTH)
	...
	void HERMES_encrypt_blocks(
	  HERMES_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void HERMES_decrypt_blocks(
	  HERMES_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
