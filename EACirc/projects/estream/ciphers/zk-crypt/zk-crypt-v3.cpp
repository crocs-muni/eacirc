...
void ECRYPT_Zkcrypt::ECRYPT_keysetup(void* ctxa, const u8* key,  u32 keysize, u32 ivsize)
{
...
void ECRYPT_Zkcrypt::ECRYPT_ivsetup(void* ctxa, const u8* iv)
{
	ZKCRYPT_ctx* ctx = (ZKCRYPT_ctx*)ctxa;
...
void ECRYPT_Zkcrypt::ZKCRYPT_process_bytes(int action, /* 0 = encrypt; 1 = decrypt; */
	void* ctxa, const u8* input, u8* output, u32 msglen)
...
DWORD runBank(ZKCRYPT_ctx* ctx, DWORD* ptrState, BYTE control, int leftIndex, int rightIndex, 
...
DWORD runSuperBank(ZKCRYPT_ctx* ctx, DWORD* ptrState, BYTE control, int leftIndex, int rightIndex,
...
void ECRYPT_Zkcrypt::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	ZKCRYPT_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Zkcrypt::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	ZKCRYPT_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}