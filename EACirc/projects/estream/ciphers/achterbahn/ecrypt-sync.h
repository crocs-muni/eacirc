...
class ECRYPT_Achterbahn: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	void ACHTERBAHN_keystream_bytes(
	  ACHTERBAHN_ctx* ctx,
	  u8* keystream,
	...
	#define ACHTERBAHN_keystream_blocks(ctx, keystream, blocks)            \
	  ACHTERBAHN_AE_keystream_bytes(ctx, keystream,                        \
		(blocks) * ACHTERBAHN_BLOCKLENGTH)
	...
	void ACHTERBAHN_encrypt_blocks(
	  ACHTERBAHN_ctx* ctx,
	  const u8* plaintext,
	  u8* ciphertext,
	  u32 blocks);                /* Message length in blocks. */

	void ACHTERBAHN_decrypt_blocks(
	  ACHTERBAHN_ctx* ctx,
	  const u8* ciphertext,
	  u8* plaintext,
	  u32 blocks);                /* Message length in blocks. */
	...
	void ACHTERBAHN_keystream_blocks(
	  ACHTERBAHN_AE_ctx* ctx,
	  const u8* keystream,
