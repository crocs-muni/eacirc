...
void ECRYPT_Wg::ECRYPT_keysetup(
					 void* ctxa, 
					 const u8* key, 
					 u32 keysize,                /* Key size in bits. */ 
					 u32 ivsize)                /* IV size in bits. */ 
{
	WG_ctx* ctx = (WG_ctx*)ctxa;
...
void ECRYPT_Wg::ECRYPT_encrypt_bytes(
						  void* ctxa, 
						  const u8* plaintext, 
						  u8* ciphertext, 
...
void ECRYPT_Wg::ECRYPT_decrypt_bytes(
						  void* ctxa, 
						  const u8* ciphertext, 
						  u8* plaintext, 
						  u32 msglen)                /* Message length in bytes. */ 
...
void ECRYPT_Wg::WG_keystream_bytes(
							WG_ctx* ctx,
							u8* keystream,
							u32 length)
{
