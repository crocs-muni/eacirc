...
#define DECIM_MAXKEYSIZE 128                 /* DECIM is a hardware candidate
...
/* ------------------------------------------------------------------------- */
class ECRYPT_Decim: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define DECIM_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	DECIM_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define DECIM_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	DECIM_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void DECIM_process_packet(
		int action,                 /* 0 = encrypt; 1 = decrypt; */
		DECIM_ctx* ctx, 
		const u8* iv,
		const u8* input, 
	...
	#define DECIM_keystream_blocks(ctx, keystream, blocks)            \
	  DECIM_keystream_bytes(ctx, keystream,                           \
		  (blocks) * DECIM_BLOCKLENGTH)
	...
	void DECIM_encrypt_blocks(
		DECIM_ctx* ctx, 
		const u8* plaintext, 
		u8* ciphertext, 
		u32 blocks);                /* Message length in blocks. */ 

	void DECIM_decrypt_blocks(
		DECIM_ctx* ctx, 
		const u8* ciphertext, 
		u8* plaintext, 
		u32 blocks);                /* Message length in blocks. */ 
