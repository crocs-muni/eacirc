...
/* ------------------------------------------------------------------------- */
class ECRYPT_Mag: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define MAG_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  MAG_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define MAG_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  MAG_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void MAG_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  MAG_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define MAG_keystream_blocks(ctx, keystream, blocks)            \
	  MAG_keystream_bytes(ctx, keystream,                           \
		(blocks) * MAG_BLOCKLENGTH)
	...
	void MAG_encrypt_blocks(
	  MAG_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void MAG_decrypt_blocks(
	  MAG_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
