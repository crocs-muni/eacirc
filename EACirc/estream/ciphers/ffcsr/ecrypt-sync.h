...
/* ------------------------------------------------------------------------- */
class ECRYPT_FFCSR: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define FFCSR_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  FFCSR_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define FFCSR_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  FFCSR_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void FFCSR_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  FFCSR_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define FFCSR_keystream_blocks(ctx, keystream, blocks)            \
	  FFCSR_keystream_bytes(ctx, keystream,                           \
		(blocks) * FFCSR_BLOCKLENGTH)
	...
	void FFCSR_encrypt_blocks(
	  FFCSR_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void FFCSR_decrypt_blocks(
	  FFCSR_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
