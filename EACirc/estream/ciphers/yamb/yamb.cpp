...
void ECRYPT_Yamb::ECRYPT_keysetup(
   void* ctxa,
   const u8*   key,
   u32         keysize,
   u32         ivsize )
...
void ECRYPT_Yamb::ECRYPT_ivsetup(
   void* ctxa, 
   const u8*   iv )
{
	YAMB_ctx* ctx = (YAMB_ctx*)ctxa;
...
void ECRYPT_Yamb::YAMB_keystream_bytes(
   YAMB_ctx* ctx,
   u8*         keystream,
   u32         length )
{
...
void ECRYPT_Yamb::YAMB_keystream_blocks(
   YAMB_ctx* ctx,
   u8*         keystream,
   u32         blocks )
{
...
void ECRYPT_Yamb::ECRYPT_encrypt_bytes(void* ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen) {
	YAMB_process_bytes(0, ctx, plaintext, ciphertext, msglen);
}
void ECRYPT_Yamb::ECRYPT_decrypt_bytes(void* ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen) {
	YAMB_process_bytes(1, ctx, ciphertext, plaintext, msglen);
}