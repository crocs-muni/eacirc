...
/* ------------------------------------------------------------------------- */
class ECRYPT_Yamb: public EstreamInterface
{
public:
	/* Mandatory functions */
	...
	#define YAMB_encrypt_packet(                                        \
		ctx, iv, plaintext, ciphertext, mglen)                            \
	  YAMB_process_packet(0,                                            \
		ctx, iv, plaintext, ciphertext, mglen)

	#define YAMB_decrypt_packet(                                        \
		ctx, iv, ciphertext, plaintext, mglen)                            \
	  YAMB_process_packet(1,                                            \
		ctx, iv, ciphertext, plaintext, mglen)

	void YAMB_process_packet(
	  int action,                 /* 0 = encrypt; 1 = decrypt; */
	  YAMB_ctx* ctx, 
	  const u8* iv,
	  const u8* input, 
	...
	#define YAMB_keystream_blocks(ctx, keystream, blocks)            \
	  YAMB_keystream_bytes(ctx, keystream,                           \
		(blocks) * YAMB_BLOCKLENGTH)
	...
	void YAMB_encrypt_blocks(
	  YAMB_ctx* ctx, 
	  const u8* plaintext, 
	  u8* ciphertext, 
	  u32 blocks);                /* Message length in blocks. */ 

	void YAMB_decrypt_blocks(
	  YAMB_ctx* ctx, 
	  const u8* ciphertext, 
	  u8* plaintext, 
	  u32 blocks);                /* Message length in blocks. */ 
