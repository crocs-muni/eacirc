...
class ECRYPT_Cryptmt:public EstreamInterface
{
public:
	/* Mandatory functions */
	 ...
	 * above.
	 */
	void ECRYPT_keysetup(void * ctx, const u8 * key, u32 keysize,	/* Key size in bits. */
	 ...
	 * IV's.
	 */
	void ECRYPT_ivsetup(void * ctx, const u8 * iv);
	...
	void ECRYPT_encrypt_bytes(void * ctx, const u8 * plaintext, u8 * ciphertext, u32 msglen);	/* Message length in bytes. */

	void ECRYPT_decrypt_bytes(void * ctx, const u8 * ciphertext, u8 * plaintext, u32 msglen);	/* Message length in bytes. */
	...
	void CRYPTMT_keystream_bytes(CRYPTMT_ctx * ctx, u8 * keystream, u32 length);	/* Length of keystream in bytes. */
	...
	void CRYPTMT_encrypt_packet(CRYPTMT_ctx * ctx,
				   const u8 * iv,
				   const u8 * plaintext,
				   u8 * ciphertext, u32 msglen);

	void CRYPTMT_decrypt_packet(CRYPTMT_ctx * ctx,
				   const u8 * iv,
				   const u8 * ciphertext,
	...
	#define CRYPTMT_encrypt_blocks(ctx, plaintext, ciphertext, blocks)  \
	  CRYPTMT_encrypt_bytes(ctx, plaintext, ciphertext,                 \
		(blocks) * CRYPTMT_BLOCKLENGTH)

	#define CRYPTMT_decrypt_blocks(ctx, ciphertext, plaintext, blocks)  \
	  CRYPTMT_decrypt_bytes(ctx, ciphertext, plaintext,                 \
		(blocks) * CRYPTMT_BLOCKLENGTH)

	#ifdef CRYPTMT_GENERATES_KEYSTREAM

	#define CRYPTMT_keystream_blocks(ctx, keystream, blocks)            \
	  CRYPTMT_keystream_bytes(ctx, keystream,                        \
		(blocks) * CRYPTMT_BLOCKLENGTH)
	...
	void CRYPTMT_encrypt_blocks(CRYPTMT_ctx * ctx, const u8 * plaintext, u8 * ciphertext, u32 blocks);	/* Message length in blocks. */

	void CRYPTMT_decrypt_blocks(CRYPTMT_ctx * ctx, const u8 * ciphertext, u8 * plaintext, u32 blocks);	/* Message length in blocks. */

	#ifdef CRYPTMT_GENERATES_KEYSTREAM

	void CRYPTMT_keystream_blocks(CRYPTMT_ctx * ctx, u8 * keystream, u32 blocks);	/* Keystream length in blocks. */
