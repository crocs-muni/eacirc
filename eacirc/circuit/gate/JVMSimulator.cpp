...
int JVMSimulator::emulate_ins(struct Pc* PC, int& returnValue) {
    ...
    case ILOAD:
    {
                  ...
                  if ( index >= MAX_NUMBER_OF_VARIABLES || m_localsUsed[index] == false ) {
                      return CONTINUE;
                  }
    ...
    case AALOAD:
    {
                   ...
                   pop_arrayref(ref);
                   if ( m_globalArraysUsed[ref] == false ) {
                       push_int(index);
                       push_arrayref(ref);
