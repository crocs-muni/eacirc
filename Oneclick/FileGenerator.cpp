...
void FileGenerator::generateFiles() {
	...
	//Creating directory for generated configuration files
	try {
		fs::create_directory(DIRECTORY_CFGS);
	} catch(fs::filesystem_error e) {
		throw std::runtime_error("can't create config directory: " + (std::string)e.what());
	}
	...
	for(int i = 0 ; i < numGenerations.size() ; i++) {
		for(int k = 0 ; k < algorithmsRounds.size() ; k++) {
			for(int l = 1 ; l < algorithmsRounds[k].size() ; l++) {
				...
				wuName = (wuName + Utils::getDate() + "_eacirc_" + projectName + "_alg" + Utils::itostr(algorithmsRounds[k][0]) +
					+ "_r" + Utils::itostr(algorithmsRounds[k][l]) + "_gen" + Utils::itostr(numGenerations[i]));

				//wuName = (wuIdentifier + "_eacirc_" + projectName + "_" + Utils::itostr(algorithmsRounds[k][0]) + "_" + algorithmName +
				//	+ "_r" + Utils::itostr(algorithmsRounds[k][l]) + "_" + Utils::itostr(numGenerations[i]) + "-gen");
