...
sha3_stream::sha3_stream(const json& config)
    ...
    , _counter(8)
//    , _heatmap(config.count("heatmap") != 0 ? std::uint64_t(config.at("heatmap")) : 0x0u)
    , _hasher(sha3_factory::create(_algorithm, unsigned(_round), config.count("heatmap") != 0 ? std::uint64_t(config.at("heatmap")) : 0x0u)) {
