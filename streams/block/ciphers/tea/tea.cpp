...
namespace block {
    ...
    tea::encrypt(const std::uint8_t* plaintext, std::uint8_t* ciphertext) {
        ...
        for (unsigned int i = 0; i < _msglen; i += 8) {
            // copy plaintext of std::uint8_t to array of std::uint32_t
            for (int j = 0; j < 2; j++)
                input[j] = u8_to_u32_copy(plaintext + i + 4 * j);
            ...
            for (int j = 0; j < 2; j++)
                u32_to_u8_copy(ciphertext + i + 4 * j, input[j]);
        }
    }
    ...
    tea::decrypt(const std::uint8_t* ciphertext, std::uint8_t* plaintext) {
        ...
        for (unsigned int i = 0; i < _msglen; i += 8) {
            // copy plaintext of std::uint8_t to array of std::uint32_t
            for (int j = 0; j < 2; j++)
                input[j] = u8_to_u32_copy(ciphertext + i + 4 * j);
            ...
            for (int j = 0; j < 2; j++)
                u32_to_u8_copy(plaintext + i + 4 * j, input[j]);
        }
    }
}
